...
import { X, Check, Plus, Shuffle, Users, ChevronDown, Gamepad } from "lucide-react";
...
const CourseGames = () => {
	...
	useEffect(() => {
		const fetchOutgoingChallenges = async () => {
			try {
				...
				setOutgoingChallengeList(
					json.challenges.map((c) => {
						let challenger = memberList.find(
							(member) => parseInt(member.userId) === parseInt(c.challengerId)
						);
		...
		const fetchIncomingChallenges = async () => {
			try {
				...
				setIncomingChallengeList(
					json.challenges.map((c) => {
						let challenger = memberList.find(
							(member) => parseInt(member.userId) === parseInt(c.contenderId)
						);
						return { ...c, name: `${challenger.firstName} ${challenger.lastName}` };
					})
				);
		...
		const fetchGameData = async () => {
			try {
				const response = await fetch(`/api/course/${course.courseId}/gameData`, {
					method: "GET",
					headers: {
						"Content-Type": "application/json",
						Authorization: `Bearer ${user.token}`,
	...
	const handleCreateChallenge = async (e) => {
		...
		try {
			...
			setOutgoingChallengeList((prev) => [
				...prev,
				{ ...json.challenge, name: `${challenger.firstName} ${challenger.lastName}` },
			]);
	...
	const handleRandomChallenge = async () => {
		try {
			...
			setOutgoingChallengeList((prev) => [
				...prev,
				{ ...json.challenge, name: `${choice.firstName} ${choice.lastName}` },
			]);
		} catch (err) {
			console.error(err);
		}
							...
							<Accordion defaultExpanded>
								...
								<AccordionDetails>
									<Stack direction="column" spacing={2} alignItems="start" key={gameList}>
										...
										<Typography variant="p">Active Players: {gameData.totalPlayers}</Typography>
									</Stack>
								</AccordionDetails>
							</Accordion>
						</Paper>
						...
						<Accordion defaultExpanded>
							...
							<AccordionDetails>
								<GameList
									...
									history={true}
									key={gameList + 1}
									refreshChallenges={triggerChallengeRefresh}
									refreshGames={() => setGameList((prev) => prev + 1)}
								/>
							</AccordionDetails>
						</Accordion>
					</Paper>
